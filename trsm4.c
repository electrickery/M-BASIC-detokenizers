/* 
   ...
   Originally written by Christian A. Ratliff
   Modified for use with TRS-80 Model 4 Basic by Edmond Orignac 

   This file is part of the model 4 Basic translator.
   ...
   along with mbasic; see the file COPYING.  If not, write to the Free
...
char *gi_show(FILE *fp)
{
  ...
  for (b = fgetc(fp); !feof(fp); b = fgetc(fp)) {
    ...
    case 246:/* / operation */ 
    case 247: /* ^ operation */ 
    case 240: { /* this is a numeric operation, try to read a number */
      int c;
      if ((c = fgetc(fp)) > 0x80) {
	ungetc(c, fp);
